# Veil Wallet (PLANE 5) — production-grade dApp

A real web wallet for the Veil shielded pool. It generates **real Groth16 proofs
...
## What it does
- **Create / import** a Veil identity (32-byte seed → key hierarchy).
...
- **`src/lib/crypto.ts`** — the browser's single source of note math:
  `circomlibjs` Poseidon (== circomlib == `veil-crypto`) + `@noble` (x25519,
...
the browser crypto matches `veil-crypto`/the circuit and that a real deposit
