...
//!
//! `#[ignore]`d because it needs the circuit artifacts (run the circuits plane's
//! `setup.sh` first). Run with: `cargo test -p veil-sdk --test e2e_prove -- --ignored`.

use std::path::PathBuf;
use std::process::Command;

use ark_bn254::Fr;
use veil_sdk::prove::{prove, ProverConfig, SnarkjsInvocation};
use veil_sdk::{Keys, Wallet};
...
fn wallet_witness_proves_and_verifies() {
    ...
    let work = std::env::temp_dir().join("veil-sdk-e2e-prove");
