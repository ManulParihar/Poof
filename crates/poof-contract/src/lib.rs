#![no_std]
//! # poof-contract — the Soroban shielded-pool contract.
//!
...
//!
//! The single state-mutating entry point is [`PoofContract::transact`], which
//! follows strict **validate -> verify -> effect -> emit** ordering:
//! no storage is mutated until the proof has passed, so a later
...
impl PoofContract {
    ...
    pub fn transact(
        ...
        // 4. Settle public amount against the tx's currency.
